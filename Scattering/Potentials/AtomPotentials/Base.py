...
class AtomPotentialGenerator:
	...
	def potential_r(self, Z, *x):
		return self.potential_from_ff(self.form_factors_r(Z, *x), *x)

	def potential_k(self, Z, *x):
		...
		return self.potential_from_ff(abs(FT.mifft(self.form_factors_k(Z, *k)))/(area/size), *x)
