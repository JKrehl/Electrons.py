...
class FresnelKernel3(Kernel):
	...
	def calc(self):
		...
		prop = FT.ifft(propf, axes=(1,2)).astype(self.dtype)

		prop = scipy.ndimage.filters.convolve1d(prop.real, 1/ssw*numpy.ones(ssw, prop.dtype), axis=0) +1j*scipy.ndimage.filters.convolve1d(prop.imag, 1/ssw*numpy.ones(ssw, prop.dtype), axis=0)
		...
		for iz, zi in Progress(enumerate(self.dz), self.idz.size, True):
			...
			for it, ti in Progress(enumerate(self.t), self.t.size):
				...
				vmemax = numpy.sqrt(numpy.abs(numpy.pi/self.k*2*max(wmx,-wmn)+dyx**2-zi**2))
